...
const formatTradingSystem = (data) => ({
    ...data,
...
const getExistingSystem = async (supabase) => {
...
const updateSystemRow = async (supabase, id, payload) => {
    const { data, error } = await supabase
        .from("trading_system")
        .update(payload)
        .eq("id", id)
...
export const fetchTradingSystem = async (req, res) => {
...
export const createTradingSystem = async (req, res) => {
    const supabase = supabaseFromReq(req);
    const body = req.body;
    try {
        ...
        return res.status(201).json(formatTradingSystem(data));
    }
    catch (error) {
        return res.status(500).json({
...
export const updateTradingSystem = async (req, res) => {
    const supabase = supabaseFromReq(req);
    const body = req.body;
    try {
        const existing = await getExistingSystem(supabase);
...
export const removeTradingPair = async (req, res) => {
    const supabase = supabaseFromReq(req);
    const { pair } = req.body;
    try {
        const existing = await getExistingSystem(supabase);
...
export const updateTradingPairs = async (req, res) => {
    const supabase = supabaseFromReq(req);
...
export const addTradingStep = async (req, res) => {
    const supabase = supabaseFromReq(req);
    const { step } = req.body;
    console.log(step);
    try {
...
export const removeTradingStep = async (req, res) => {
    const supabase = supabaseFromReq(req);
...
export const updateTradingSteps = async (req, res) => {
    const supabase = supabaseFromReq(req);
...
export const addTradingCriterion = async (req, res) => {
    const supabase = supabaseFromReq(req);
    ...
    try {
        ...
        const alreadyExists = currentCriteria.some((item) => item.id === nextCriterion.id);
...
export const removeTradingCriterion = async (req, res) => {
    const supabase = supabaseFromReq(req);
    const { id } = req.body;
    try {
        const existing = await getExistingSystem(supabase);
...
export const updateTradingCriteria = async (req, res) => {
    const supabase = supabaseFromReq(req);
